...
```bash
InsuranceProject$ sam build --use-container
```
...
```bash
InsuranceProject$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
InsuranceProject$ sam local start-api
InsuranceProject$ curl http://localhost:3000/
```
...
```bash
InsuranceProject$ sam logs -n HelloWorldFunction --stack-name InsuranceProject --tail
```
...
```bash
InsuranceProject$ pip install -r tests/requirements.txt --user
# unit test
InsuranceProject$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
InsuranceProject$ AWS_SAM_STACK_NAME="insuranceproject" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "insuranceproject"
```
